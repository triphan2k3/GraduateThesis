...
class POPOP:
    def __init__(self, params):
        ...
        self.important_pixels = self.params["important_pixels"]
        self.p = np.array(
            [
                (1 - self.params["pr0"]) / 2,
                (1 - self.params["pr0"]) / 2,
    ...
    def mutation1(
        ...
        prob = np.random.random()  # decide whether to choose an elite in archive for mutation or generate a new one
        ...
        mutation_mode_prob = np.random.random()
        if mutation_mode_prob > 0.5: # x is here
            '''
            there is x% chance of the mutation will select the pixels randomly like KeLi
            '''
            ...
            '''
            otherwise, the mutation will select the pixels like the initialization
            '''
            T = np.arange(self.locations[obj_idx])
            prob_T = self.important_pixels[obj_idx].copy()
            prob_T[M_a] = 0
            prob_T = prob_T / prob_T.sum()
            sizeAB = int(min(self.params["pm"] * max_modified, len(T) - len(M_a)))
            B_important_based = np.random.choice(T, sizeAB // 2, replace=False, p = prob_T)
            B_random_based = np.random.choice(np.setdiff1d(T, np.union1d(B_important_based, M_a)),
                                              size = sizeAB - B_important_based.shape[0],
                                              replace = False)
            B = np.concatenate([B_important_based, B_random_based])
        #################################################################################
...
#               print(j)
    ...
    def run(self, n_gen=1000):
        parents = []
        '''
        Initialize: for each individual in population
        + 50% of pixels are randomly generated
        + 50% of pixels are chosen from important pixels probs
        '''
        for _ in range(self.params["population_size"]):
            pos = []
            values = []
            for i, max_modified in enumerate(self.max_modified):
                important_based_pos = np.random.choice(self.locations[i], size=max_modified // 2, replace=False, p = self.important_pixels[i])
                random_based_pos = np.random.choice(np.setdiff1d(np.arange(self.locations[i]), important_based_pos),
                                                    size=max_modified - important_based_pos.shape[0],
                                                    replace=False)
                pos.append(
                    np.concatenate([important_based_pos, random_based_pos])
                )
        ...
        '''
        save the conf_scores in the previous generation for each object in order to calculate the loss function
        '''
        ...
        for i in range(n_gen):
            ...
            # adv = []
            # for item in pool:
            #     fitnesses.append(item.loss)
            #     adv.append(item.is_adv)
            # fitnesses.sort()
            # print('[', end='')
            # for item, status in zip(fitnesses, adv):
            #   print('({:.3f}, {})'.format(item, status), end=', ')
            # print(']')
            ...
            ###############################################################################################
            '''
            this version doesn't use self.rearrange()
